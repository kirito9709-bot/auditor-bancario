...
# ─────────────────────────────────────────────
# 1. MOTOR PDF — PARSER BBVA ESPECÍFICO
# ─────────────────────────────────────────────

MESES_MX = "ENE|FEB|MAR|ABR|MAY|JUN|JUL|AGO|SEP|SET|OCT|NOV|DIC"

# Fecha tipo BBVA: "19/JUN", "19/JUN/2024", "19-JUN", "19/08", "19/08/24"
RE_FECHA = re.compile(
    rf"^(\d{{1,2}}\s*[/\-]\s*(?:{MESES_MX}|\d{{1,2}})(?:\s*[/\-]\s*\d{{2,4}})?)\b",
    re.IGNORECASE,
)

# Monto: "1,033.85", "$304.00", "(1,234.56)", "-304.00"
RE_MONTO = re.compile(r"\(?-?\$?\s?(?:[0-9]{1,3}(?:,[0-9]{3})*|[0-9]+)\.[0-9]{2}\)?")

# Filas de RESUMEN FINANCIERO: saldos totales, GAT, CAT, ISR — nunca son movimientos
TERMINOS_RESUMEN = [
    "saldo anterior",
    "saldo final",
    "saldo promedio",
...
# Líneas de ruido: encabezados, pies de página, texto legal, QR binario
TERMINOS_RUIDO = [
    "estado de cuenta",
    "página",
    "pagina",
    ...
    "banco de mexico",
    "condusef",
    "(cid:",  # basura binaria del sello/QR
    "estimado cliente",
    "aviso de privacidad",
...
# Líneas de referencia SPEI (números de cuenta / CLABE / rastreo) — parte del concepto
# pero no deben iniciar un nuevo bloque
RE_SOLO_NUMEROS = re.compile(r"^[\d\s\-/]+$")
RE_CLABE = re.compile(r"\b\d{18}\b")
RE_REFERENCIA_SPEI = re.compile(
    r"^(RFC:|AUT:|Referencia|0240|0241|0242|\d{10,}|ALMA |ROMUALDO |RAYMUNDO )",
    re.IGNORECASE,
)
...
def clasificar_bbva(texto: str) -> str:
    t = texto.lower()
    if any(
        k in t
        for k in [
            ...
            "recibido",
            "rendimiento",
            "pago cuenta de tercero",  # abono entrante
        ]
    ):
        return "Ingreso"
    return "Egreso"
...
def _procesar_bloques_bbva(texto: str, origen: str) -> list:
    """
    Parser de texto plano para estados de cuenta BBVA.

    Formato de línea de movimiento:
        DD/MMM  DD/MMM  DESCRIPCION  [CARGO|ABONO]  [SALDO_OP  SALDO_LIQ]

    Reglas aplicadas:
    - La primera fecha es la de operación (la usamos).
    - La segunda fecha es la de liquidación (la descartamos del concepto).
    - Solo tomamos el PRIMER monto de la línea (el importe del movimiento).
      Los siguientes son columnas de saldo y se descartan.
    - Las líneas que comienzan con RFC:, AUT:, Referencia, o son CLABE/rastreo
      se acumulan como continuación del concepto actual (no como nuevo bloque).
    - Líneas de ruido (pie de página, QR binario, aviso legal) se descartan.
    - Filas de resumen financiero (Saldo Anterior, Total, GAT…) se descartan.
    """
    filas = []
    bloque_fecha = None
    bloque_lineas = []
    ...
    def cerrar_bloque():
        if bloque_fecha is None or not bloque_lineas:
            return
        texto_bloque = " ".join(bloque_lineas)
        # Solo el PRIMER monto es el importe real
        montos = RE_MONTO.findall(texto_bloque)
        if not montos:
            return
        monto = _limpiar_monto(montos[0])
        if not monto or monto < 1:  # ignora montos < $1 (e.g. 0.00)
            return
        # Limpiar el concepto quitando TODOS los montos detectados
        concepto = RE_MONTO.sub("", texto_bloque).strip(" -:")
        concepto = re.sub(r"\s{2,}", " ", concepto)
        # Descartar si es fila de resumen financiero
        if _es_resumen(f"{bloque_fecha} {concepto}"):
            return
        tipo = clasificar_bbva(concepto)
        filas.append(
            {
                "Origen": origen,
                "Fecha": bloque_fecha,
    ...
    for linea in texto.split("\n"):
        ...
        if not linea:
            continue
        # Descartar ruido de forma absoluta
        if _es_ruido(linea):
            continue
        # Descartar líneas de solo números de cuenta/CLABE/rastreo
        if RE_SOLO_NUMEROS.match(linea) or RE_CLABE.search(linea):
            continue
        ...
        if m_fecha:
            ...
            resto = linea[m_fecha.end() :].strip()

            # BBVA pone DOS fechas: OPER + LIQ — eliminar la segunda si existe
            m_fecha2 = RE_FECHA.match(resto)
            if m_fecha2:
                resto = resto[m_fecha2.end() :].strip()

            bloque_lineas = [resto] if resto else []
        elif bloque_fecha is not None:
            # Línea de continuación: RFC, AUT, Referencia, nombre del emisor, etc.
            # Las incluimos solo si NO son ruido
...
def _extraer_de_tabla_bbva(tabla, num_pagina: int) -> list:
    """
    Extrae movimientos de tablas detectadas por pdfplumber.

    IMPORTANTE: Solo procesa tablas que tengan un encabezado reconocible
    con 'fecha' y ('concepto' o 'descripcion'). Si la tabla no tiene ese
    encabezado, se descarta para no confundirla con tablas de resumen
    financiero (GAT, Saldo Anterior, etc.).
    """
    if not tabla:
        return []
    ...
    for i, fila in enumerate(tabla[:4]):
        celdas = [str(c).strip().lower() if c else "" for c in fila]
        texto_fila = " ".join(celdas)
        if "fecha" in texto_fila and (
            "concepto" in texto_fila or "descrip" in texto_fila
        ):
            fila_encabezado = i
    ...
    # Sin encabezado válido → NO procesar (evita parsear tablas de resumen)
    if fila_encabezado < 0:
        return []

    filas = []
    for fila in tabla[fila_encabezado + 1 :]:
        ...
        # Buscar monto y tipo
        monto_txt, tipo = None, None
        if (
            idx_cargo is not None
            and idx_cargo < len(celdas)
            and RE_MONTO.search(celdas[idx_cargo])
        ):
            monto_txt, tipo = celdas[idx_cargo], "Egreso"
        elif (
            idx_abono is not None
            and idx_abono < len(celdas)
            and RE_MONTO.search(celdas[idx_abono])
        ):
            monto_txt, tipo = celdas[idx_abono], "Ingreso"
        elif (
            idx_monto is not None
            and idx_monto < len(celdas)
            and RE_MONTO.search(celdas[idx_monto])
        ):
        ...
        if not monto or monto < 1:
            continue

        # Concepto
        partes = []
        if idx_concepto is not None and idx_concepto < len(celdas):
            partes = [celdas[idx_concepto]]
        else:
            skip = {idx_fecha, idx_cargo, idx_abono, idx_monto}
            for j, c in enumerate(celdas):
                if j in skip or not c or RE_MONTO.fullmatch(c):
                    continue
                partes.append(c)
        concepto = " ".join(p for p in partes if p).strip()

        if _es_resumen(f"{fecha} {concepto}"):
            continue
...
def extraer_transacciones_pdf(file_pdf):
    ...
    Estrategia por página:
      1. Tablas con encabezado válido (más confiable cuando el PDF las tiene)
      2. Texto plano con parser de bloques BBVA (dos fechas, saldo ignorado)
      3. OCR si la página no tiene texto embebido
    """
    rows, paginas_ocr_texto, paginas_sin_texto = [], {}, []
    ...
    with pdfplumber.open(BytesIO(pdf_bytes)) as pdf:
        for num, page in enumerate(pdf.pages, 1):
            ...
            # 1) Tablas con encabezado reconocible
            try:
                tablas = page.extract_tables() or []
            except Exception:
                tablas = []
            for tabla in tablas:
                filas_pagina.extend(_extraer_de_tabla_bbva(tabla, num))

            # 2) Texto plano (siempre corre; complementa tablas si las hay)
            txt = page.extract_text() or ""
            if txt.strip():
                filas_texto = _procesar_bloques_bbva(txt, f"Pág.{num}")
                # Evitar duplicados exactos con lo ya extraído de tablas
                claves_tabla = {
                    (f["Fecha"], round(f["Monto"], 2)) for f in filas_pagina
                }
                for f in filas_texto:
                    if (f["Fecha"], round(f["Monto"], 2)) not in claves_tabla:
                        filas_pagina.append(f)
            else:
                paginas_sin_texto.append(num)

            rows.extend(filas_pagina)

    # 3) OCR para páginas escaneadas
    if paginas_sin_texto and OCR_DISPONIBLE:
        with st.spinner(f"🔍 Aplicando OCR en {len(paginas_sin_texto)} página(s)..."):
            imgs = convert_from_bytes(pdf_bytes, dpi=300)
            for i, img in enumerate(imgs, 1):
                if i in paginas_sin_texto:
                    texto_ocr = pytesseract.image_to_string(img, lang="spa")
                    paginas_ocr_texto[i] = texto_ocr
                    rows.extend(_procesar_bloques_bbva(texto_ocr, f"OCR Pág.{i}"))
    ...
    if not df.empty:
        df = df.drop_duplicates(subset=["Fecha", "Concepto", "Monto"]).reset_index(
            drop=True
        )
